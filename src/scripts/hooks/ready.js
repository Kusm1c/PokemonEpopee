...
export const Ready = {
    listen() {
        Hooks.once("ready", () => {
            ...
            if(game.world.id === "ptr-dev-world") testWorldSettings();
...
function testWorldSettings() {
    Hooks.on("renderSettings", (settings, $html, _) => {
        $html.find('#settings-game').prepend("<h4>Settings Disabled on Test World</h4>")
        $html.find('#settings-game').find("button").attr("disabled", true);
        $html.find('#settings-game').find("button[data-action='players']").remove()
    });
    Hooks.on("changeSidebarTab", (_) => {
        $('#settings-game').prepend("<h4>Settings Disabled on Test World</h4>")
        $('#settings-game').find("button").attr("disabled", true);
        $('#settings-game').find("button[data-action='players']").remove()
    })
}
